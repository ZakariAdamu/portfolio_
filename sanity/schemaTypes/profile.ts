...
export default defineType({
	...
	fields: [
		...
		defineField({
			...
			type: "string",
			description: "Highly Skilled Frontend Developer",
			validation: (Rule) => Rule.required(),
		}),
		defineField({
		...
		defineField({
			...
			type: "array",
			description:
				"The flip/animated words in your headline e.g. amazing web applications, interactive user experiences, responsive designs",
			of: [{ type: "string" }],
		...
		defineField({
			...
			type: "text",
			rows: 3,
			description: "Brief introduction about yourself (2-3 sentences)",
			validation: (Rule) => Rule.required(),
		}),
		defineField({
			name: "fullBio",
			...
			fields: [
				{
					...
					description:
						"A short description of the image for SEO and accessibility",
					validation: (Rule) => Rule.required(),
				},
			],
		}),
